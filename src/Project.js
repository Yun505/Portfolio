import Accordion from 'react-bootstrap/Accordion';
import Card from 'react-bootstrap/Card';

export default function Project() {
    return (
        <div className="container pl-20 pr-20">
            <br></br>
            <br></br>
            <h1 className="text-5xl">Projects</h1>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Current Project: Alumni Portal</Card.Title>
                    <Card.Text>
                        Prototype Demo for clients to give feedback on the portal's functions.
                        We are currently working with administration to create the first ever BCA-Alumni Portal.
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="/https://github.com/RemingtonKim/AcademiesAlumni">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>AP Microeconomics Dating Simulation</Card.Title>
                    <Card.Text>
                        
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/MicroDatingSimulation">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Natto Obstacle Course</Card.Title>
                    <Card.Text>
                        aksfjkl;sdjf;sdjfksd;fj
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/Natto_Obstacle_Course">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Music Classifier</Card.Title>
                    <Card.Text>
                        ksafjl;kasfjklasjflks;df
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/MusicClassifier">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>BlackJack GUI</Card.Title>
                    <Card.Text>
                        akljfkl;sjfkl;asdjfsdf
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/Midyear-Project-Blackjack">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Rocket Game</Card.Title>
                    <Card.Text>
                        sadfjlkasdfj;lksdjfksdf
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/rocket">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Experimental: Music / CSS Website</Card.Title>
                    <Card.Text>
                        alskjf;lsadf
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="https://github.com/Yun505/MakeAWebPageProject">GitHub</Card.Link>
                </Card.Body>
            </Card>
            <br></br>
            <Card style={{ width: '450' }}>
                <iframe width="stretch" height="315" src="https://www.youtube.com/embed/xNRJwmlRBNU" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                <Card.Body>
                    <Card.Title>Librosa Model</Card.Title>
                    <Card.Text>
                        asdfkljsdklfj;sf
                    </Card.Text>
                </Card.Body>
                <Card.Body>
                    <Card.Link href="/https://github.com/RemingtonKim/AcademiesAlumni">GitHub</Card.Link>
                </Card.Body>
            </Card>
        </div>
    );
}